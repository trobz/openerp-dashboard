...
VIEW_TYPE = ('dashboard', _('Dashboard'))
...
def valid_node_group(node):
    ...
    if not valid_type_dashboard(node):
        res = False

    return res


def valid_type_dashboard(arch, fromgroup=True):
    return True
...
class IrUiView(osv.Model):
    ...
    def _check_xml_dashboard(self, cr, uid, ids, context=None):
        domain = [
            ('id', 'in', ids),
            ('type', '=', VIEW_TYPE[0]),
        ]
        ...
        for view in self.browse(cr, uid, view_ids, context=context):
            ...
            for view_arch in view_docs:
                if not valid_type_dashboard(view_arch, fromgroup=False):
                    return False
    ...
    _constraints = [
        (
            _check_xml_dashboard,
            'Invalid XML for dashboard view architecture',
            ['arch'],
        ),
    ]
